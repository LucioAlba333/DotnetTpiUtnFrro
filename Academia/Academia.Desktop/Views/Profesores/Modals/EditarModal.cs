...
public partial class EditarModal : Form
{
    ...
    private async Task CargaDatos()
    {
        ...
        {
            _estaCargando = true;
            List<PersonaDto> personas = (List<PersonaDto>)await PersonaApiClient.GetProfesoresAsync();
    ...
    private async void buttonEnviar_Click(object sender, EventArgs e)
    {
        try
        {
            if (this.comboBox2.SelectedItem is PersonaDto profesor)
            {
                ...
                profesor.FechaNacimiento = dateTimePickerNacimiento.Value.Date;
                await PersonaApiClient.UpdateAsync(profesor);
